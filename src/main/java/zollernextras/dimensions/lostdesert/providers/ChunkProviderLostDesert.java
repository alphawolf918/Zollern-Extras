...
import zollernextras.dimensions.lostdesert.WorldGenLostDesertDungeons;
...
public class ChunkProviderLostDesert extends ChunkProviderGenerate {
	final Block topBlockID = BlockList.wonderSand;
	final byte topBlockMeta = 5;
	final Block fillBlockID = BlockList.creepStone;
	final byte fillBlockMeta = 3;
	final Block lowerBlockID = BlockList.wonderStone;
	...
	private BiomeGenBase[] biomesForGeneration = { BiomeList.lostDesert };
	...
	public ChunkProviderLostDesert(World par1World, long par2, boolean par4) {
	...
	public void generateTerrain(int chunkX, int chunkZ, Block[] idArray,
		...
		for (int x = 0; x < ChunkProviderLostDesert.CHUNK_SIZE_X; x++) {
			for (int z = 0; z < ChunkProviderLostDesert.CHUNK_SIZE_Z; z++) {
				...
				for (int y = 0; y < ChunkProviderLostDesert.CHUNK_SIZE_Y; y++) {
					if (y < ChunkProviderLostDesert.MID_HEIGHT + yDev) {
	...
	public void populate(IChunkProvider par1IChunkProvider, int par2, int par3) {
		...
		for (k1 = 0; doGen && k1 < 8; ++k1) {
			...
			new WorldGenLostDesertDungeons().generate(this.worldObj, this.rand,
					l1, i2, j2);
		}
	...
	public List getPossibleCreatures(EnumCreatureType par1EnumCreatureType,
		...
		if (par1EnumCreatureType == EnumCreatureType.monster) {
			...
			monsters.add(new SpawnListEntry(EntityMegaCreeper.class, 8, 2, 3));
			monsters.add(new SpawnListEntry(EntityShadowSkeleton.class, 8, 2, 3));
			monsters.add(new SpawnListEntry(EntityMummy.class, 8, 2, 3));
			monsters.add(new SpawnListEntry(EntityScorpion.class, 8, 2, 3));
			return monsters;
		} else {
			return null;
		}
	...
	public void recreateStructures(int par1, int par2) {
		Block[] abyte = { BlockList.shinestoneBricks };
		...
		this.villageGenerator.func_151539_a(this, this.worldObj, par1, par2,
				abyte);
		this.scatteredFeatureGenerator.func_151539_a(this, this.worldObj, par1,
				par2, abyte);
	}
}
