...
public class SwordEnder extends ZollernSword {
	 ...
	 */
	private void checkBlockAt(double posX, double posY, double posZ, World world,
			EntityPlayer entityplayer) {
		if (world.getBlockState(new BlockPos(posX, posY, posZ)) == Blocks.AIR.getDefaultState()) {
			world.playSound(entityplayer, new BlockPos(posX + 0.5D, posY + 0.5D, posZ + 0.5D),
					SoundEvents.ENTITY_ENDERMEN_TELEPORT, SoundCategory.MASTER, 2.0F,
					world.rand.nextFloat() * 0.1F + 0.9F);
			entityplayer.attemptTeleport(posX, posY, posZ);
			for (int t = 1; t < 10; t++) {
				world.spawnParticle(EnumParticleTypes.PORTAL, posX + 5.0D, posY + 5.0D,
						posZ + 5.0D, 0, 0, 0);
			}
		} else {
